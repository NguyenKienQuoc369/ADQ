...
type AuthContextValue = {
  ...
  setupGoogleRecovery: (payload: { name: string; username: string; password: string }) => Promise<void>;
...
function mapSupabaseUserToAppUser(user: SupabaseUser): AppUser {
  ...
  const hasRecoveryPassword = Boolean(metadata.has_recovery_password);
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
        ...
        if (res.status === 200) {
          ...
          if (payload?.user) {
            setUser({
              ...
              hasRecoveryPassword: payload.user.hasRecoveryPassword,
            });
            setLockMessage(null);
          }
  ...
  const setupGoogleRecovery = useCallback(async (payload: { name: string; username: string; password: string }) => {
    const supabase = getSupabaseClient();
    const { data, error } = await supabase.auth.updateUser({
      password: payload.password,
      data: {
    ...
    if (data.user) {
      const updatedUser = mapSupabaseUserToAppUser(data.user);
      updatedUser.hasRecoveryPassword = true;
      setUser(updatedUser);

      if (typeof window !== "undefined") {
        localStorage.setItem(`adq_recovery_setup_${data.user.id}`, "true");
      }
    }
  }, [getSupabaseClient]);
  ...
  const value = useMemo<AuthContextValue>(
    ...
    [user, loading, lockMessage, login, register, acceptTerms, setupGoogleRecovery, loginWithGoogle, logout, refreshUser, updateUser],
  );
