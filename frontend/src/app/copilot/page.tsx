...
function CopilotContent() {
  ...
  useEffect(() => {
    if (jobId) handleAutoAnalyze(jobId);
    else if (vulnParam && endpointParam) handleAutoPatch(vulnParam, endpointParam);
  }, [jobId, vulnParam, endpointParam]);

  const handleAutoAnalyze = async (jid: string) => {
    const id = "analyze_" + Math.random().toString(36).slice(2, 9);
    setMessages((s) => [...s, { id, sender: "user", text: `Phân tích chuyên sâu kết quả phiên quét ID: ${jid}` }]);
    setLoading(true);
    try {
      const res = await copilotAnalyze(jid);
      setMessages((s) => [...s, { id: id + "_r", sender: "copilot", text: safeString(res.analysis || "Đã phân tích xong dữ liệu quét.") }]);
    } catch {
      setMessages((s) => [...s, { id: id + "_r", sender: "copilot", text: "Không thể nạp dữ liệu phân tích từ Backend." }]);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAutoPatch = async (vuln: string, ep: string) => {
    ...
    setMessages((s) => [...s, { id, sender: "user", text: `Tạo mã vá One-Click cho lỗ hổng: ${vuln} tại endpoint: ${ep}` }]);
    setLoading(true);
    try {
      ...
      setMessages((s) => [...s, { id: id + "_r", sender: "copilot", text: "Tạo mã vá thất bại. Vui lòng thử lại." }]);
    } finally {
      setLoading(false);
    }
  };
  ...
  const send = async () => {
    ...
    try {
      ...
      setMessages((s) => [...s, { id: id + "_r", sender: "copilot", text: answer }]);
    } catch {
      setMessages((s) => [...s, { id: id + "_r", sender: "copilot", text: "Không thể kết nối đến máy chủ AI Copilot." }]);
    } finally {
      setLoading(false);
    }
  };
  ...
  const runQuickAction = async (action: string) => {
    if (action === "patch") await handleAutoPatch("CORS Misconfiguration & Exposed Endpoints", "/api/v1/user/profile");
    else if (action === "deep-scan") setText("Hãy giải thích các bước kiểm tra lỗ hổng IDOR và Race Condition trong ứng dụng web?");
    else if (action === "stress") setText("Làm thế nào để cấu hình k6 stress test 5000 RPS mà không bị Cloudflare block?");
  };
        ...
        <Card className="border border-white/[0.08] bg-slate-950/85 backdrop-blur-xl shadow-2xl">
          <CardHeader className="pb-3 border-b border-white/[0.06]">
            <div className="flex items-center justify-between">
              ...
              {target && <Badge className="border-cyan-500/40 bg-cyan-950/40 text-cyan-300 text-xs font-mono" variant="outline">Target: {target}</Badge>}
            </div>
          </CardHeader>
          <CardContent className="space-y-3 pt-3">
            <div className="h-[calc(100vh-18rem)] min-h-[400px] overflow-y-auto space-y-3.5 rounded-2xl border border-white/[0.06] bg-slate-900/60 p-4 scrollbar-thin scrollbar-thumb-slate-800">
              {messages.map((m) => (
                <div key={m.id} className={`flex ${m.sender === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[88%] rounded-2xl p-4 text-xs sm:text-sm leading-relaxed ${m.sender === "user" ? "bg-gradient-to-r from-cyan-600 to-emerald-600 text-slate-950 font-semibold shadow-md" : "bg-slate-950/90 border border-white/[0.08] text-slate-200 shadow-lg"}`}>
                    {m.sender === "user" ? (
                    ...
                    {m.toolResult?.content && (
                      <div className="mt-3 relative rounded-xl bg-slate-950 p-3 font-mono text-xs text-emerald-300 border border-slate-800">
                        <button type="button" onClick={() => handleCopy(m.toolResult.content, m.id)} className="absolute top-2 right-2 flex items-center gap-1 rounded bg-slate-800 px-2 py-1 text-[10px] text-slate-300 hover:bg-slate-700 transition">
            ...
            <div className="flex gap-2">
              <Input
                ...
                className="h-11 border-slate-800 bg-slate-900/80 text-xs sm:text-sm text-slate-100 rounded-xl"
              />
              <Button onClick={send} disabled={loading || !text.trim()} className="h-11 px-5 bg-gradient-to-r from-cyan-500 to-emerald-500 text-slate-950 font-bold text-xs rounded-xl transition">
                {loading ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4 mr-1.5" />} Gửi
              </Button>
            </div>
          </CardContent>
        </Card>
